...
import { Mic, MicOff, PhoneOff, Settings, ShieldCheck, Wifi, UserPlus, Info, AlertCircle, Eye, EyeOff, Timer } from 'lucide-react';
  ...
  return (
    <div className="h-screen w-screen bg-slate-950 overflow-hidden relative flex flex-col font-sans selection:bg-indigo-500/30 text-white">
      ...
      <header className="absolute top-0 left-0 w-full p-8 flex items-center justify-between z-30 pointer-events-none">
        <div className="flex items-center gap-6 pointer-events-auto">
          <div className="hidden md:block">
            <h2 className="font-black tracking-tight text-xl leading-tight uppercase flex items-center gap-2 drop-shadow-xl">
            ...
            <div className="flex items-center gap-3">
                 ...
                 <div className="flex items-center gap-2 opacity-60">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Secure Stream</span>
                 </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
             <div className="px-5 py-2.5 bg-black/40 backdrop-blur-3xl rounded-2xl border border-white/5 flex items-center gap-3 shadow-2xl">
                <Timer className="w-4 h-4 text-indigo-400" />
                <span className="font-mono font-black text-sm tabular-nums">{timeLeft}</span>
            </div>
      ...
      <main className="flex-1 relative overflow-hidden bg-slate-900 flex items-center justify-center">
            ...
            {/* Heuristic Attention Warning Overlay */}
            {isDistracted && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-in fade-in zoom-in duration-300">
                    <div className="bg-red-600/90 backdrop-blur-2xl p-8 rounded-[3rem] border border-white/20 shadow-2xl flex flex-col items-center gap-4">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-white/80 text-sm font-bold">Please stay within the interviewer's view.</p>
                        </div>
                    </div>
                </div>
            )}

            {/* Cinematic Subtitles (Lower Third) */}
            {currentQuestion && (
                <div className="absolute bottom-40 left-1/2 -translate-x-1/2 z-20 w-full max-w-4xl text-center px-6 transition-all duration-700">
                    <div className="bg-black/70 backdrop-blur-[40px] px-12 py-6 rounded-[2.5rem] border border-white/5 shadow-2xl">
                        <p className="text-white text-2xl font-bold leading-relaxed tracking-tight drop-shadow-xl underline decoration-indigo-500/30 decoration-4">
                            {currentQuestion}
                        </p>
                    </div>
                </div>
      ...
      {/* Meeting Control Pillar (Center) */}
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-40 w-full max-w-3xl px-6">
          <div className="bg-black/60 backdrop-blur-3xl px-8 py-5 rounded-[2.5rem] border border-white/10 shadow-3xl flex items-center justify-between gap-8 transition-all duration-500">
              ...
              <div className="hidden md:flex items-center gap-4">
                  ...
                  <div>
                      <p className="text-[10px] font-black text-white/40 uppercase tracking-widest leading-none mb-1">State</p>
              ...
              <div className="flex items-center gap-6">
                   <Button 
                     onClick={onMicToggle}
                     className={`w-14 h-14 rounded-full transition-all duration-300 shadow-xl ${isMicOn ? 'bg-white/10 hover:bg-white text-white hover:text-black' : 'bg-red-500 hover:bg-red-600 text-white'}`}
                   >
                       {isMicOn ? <Mic className="w-6 h-6" /> : <MicOff className="w-6 h-6" />}
                   </Button>

                   <Link href="/practice/interview">
                        <Button className="w-20 h-14 rounded-[1.5rem] bg-rose-600 hover:bg-rose-700 text-white shadow-2xl shadow-rose-600/20">
      ...
      {/* Floating Self-Preview (Bottom Right) */}
      <div className="absolute bottom-32 right-10 z-30 w-72 aspect-video bg-slate-900 rounded-[2rem] overflow-hidden border-2 border-white/10 shadow-3xl group">
            ...
            <div className="absolute top-4 left-4 flex items-center gap-2 px-2 py-1 bg-black/60 backdrop-blur rounded-lg">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest leading-none">
                    {isDistracted ? "Unfocused" : "Focused"}
                </span>
            </div>
      </div>
